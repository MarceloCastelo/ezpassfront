document.addEventListener("DOMContentLoaded", function () {
  ...
  async function viewSite(site) {
      ...
      const passwordCheckResult = await checkPasswordPwned(site.password);
      const passwordCheckElement = document.createElement("p");
      passwordCheckElement.innerHTML = `<strong>Verificação de Senha:</strong> ${passwordCheckResult}`;
      viewPopup.document.body.appendChild(passwordCheckElement);
  }
  ...
  async function checkPasswordPwned(password) {
      ...
      try {
          ...
          if (!response.ok) {
              throw new Error("Erro ao verificar a senha no HIBP");
          }
          const data = await response.text();
          const lines = data.split("\n");
          for (const line of lines) {
              const [hashedSuffix, count] = line.split(":");
              if (hashedSuffix.toLowerCase() === suffix.toLowerCase()) {
                  return `A senha foi exposta ${count} vezes. Considere alterá-la.`;
              }
          }
          return "A senha não foi exposta.";
      } catch (error) {
          console.error("Erro:", error);
          return "Erro ao verificar a senha. Por favor, tente novamente.";
      }
  }
